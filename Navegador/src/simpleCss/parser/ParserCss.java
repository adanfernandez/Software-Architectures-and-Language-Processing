			...
			while (token.getToken().equals(TokensIdCss.COLOR) || token.getToken().equals(TokensIdCss.TEXT_ALIGN)
				...
				if (token.getToken().equals(TokensIdCss.TEXT_ALIGN)) {
					...
					if (token.getToken().equals(TokensIdCss.CENTER)) {
						...
						errorSintactico("Se esperaba un center, right o left y se ha obtenido " + token.getLexeme(),
								token.getLine());
					}
				...
				if (token.getToken().equals(TokensIdCss.FONT_STYLE)) {
					...
					} else if (token.getToken().equals(TokensIdCss.NEGRITA)) {
						fontStyleValor = new Negrita();
					}
					else {
						errorSintactico("Se esperaba un italic o normal y se ha obtenido " + token.getLexeme(),
								token.getLine());
					}
